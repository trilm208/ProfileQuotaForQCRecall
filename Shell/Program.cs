...
namespace Shell
{
    ...
    {
        private static string CityHandle;
        private static bool IsNeedPlaceProduct;
        /// <summary>
        ...
        private static void Run(string[] args)
        {
            ...
            if (ds.FirstTable().Rows.Count == 1)
            {
                ...
                CityHandle = ds.FirstRow().Item("CityHandle");
                //IsNeedPlaceProduct = Convert.ToBoolean(ds.FirstRow().Item("IsNeedPlaceProduct"));
                ...
                else
                {
                    using (var frm = new frmProjectList())
                    {
                        ...
                        if (frm.ShowDialog() == DialogResult.OK)
                        {
                            ...
                            CityHandle = frm.SelectedCityHandle;
                            IsNeedPlaceProduct = frm.SelectedIsNeedPlaceProduct;
                        }
                    }
                }
            }
            ...
            {
                UI.ShowError("Chưa chọn dự án");
                return;
            }
            using (var frm = new frmFWRecall())
            {
                frm.Text = String.Format("Field Assignment Bạn đang làm việc với dự án : {0}", ProjectNo);
                frm.ProjectID = ProjectID;
                frm.ProjectNo = ProjectNo;
                frm.IsNeedPlaceProduct = IsNeedPlaceProduct;
                frm.Initialize(services);
                frm.Process();
                //SplashScreenManager.CloseForm();
                Application.Run(frm);
