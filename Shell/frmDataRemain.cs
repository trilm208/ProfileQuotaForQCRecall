...
namespace Shell
{
    public partial class frmDataRemain : ClientForm
    {
        public frmDataRemain()
        {
            InitializeComponent();
        }
        ...
        public void LoadQuota()
        {
            ...
            //2
            query += DataAccess.DataQuery.Create("Docs", "ws_DOC_Answers_ListCalculateQuotaRemain", new
            {
                ProjectID
            });
            //3
        ...
        public void LoadQuota(DataTable tblProfile, DataTable tblQuotaValue, DataTable tblQuotaField, DataTable tblExpression)
        {
            ...
            foreach (DataRow row in tblExpression.Rows)
            {
                ...
                if (currentCount <= minValue && minValue != maxValue)
                    row["NeedQty"] = String.Format("Còn {0}-{1}", minValue, maxValue);
                if (currentCount <= minValue && minValue == maxValue)
                ...
                if (currentCount > minValue && minValue != maxValue && currentCount > maxValue)
                    row["NeedQty"] = String.Format("Đư +{0}", currentCount - maxValue);



            }

            var dr = tblExpression.Select("MaxValue<=CurrentCount");

            foreach (DataRow r in dr)
            {
                var selectedRows = tblProfile.Select(r.Item("ConditionExpression"));
                foreach (DataRow c in selectedRows)
                    tblProfile.Rows.Remove(c);
            }
